  ...
  for (i in seq_along(likelihoods)) {
    expect_equal(
      update_gamma_parameters(
        alpha_prior, beta_prior, likelihoods[i],
        obs = OBS, s_size = s_size,
        a_obs = a_obs
      ),
      res[i, ]
    )
...
test_that("qgamma_post works", {
  ...
  expect_equal(
    qgamma_post(-2, OBS, s_size, "exp", log.p = TRUE),
